<?php
	...
	//valiadte Phone Number
	//if(empty($_POST['PhoneNumber']))
	{
		//$info['errors']['PhoneNumber'] = " Phone Number is required";
	}//else
	//if(!preg_match("/^([01]|\+88)?\d{11}/", $_POST['PhoneNumber']))
	{
		//$info['errors']['PhoneNumber'] = "Type your numbers correctly ";
	} 
	...
	if(empty($info['errors']) && $row)
	{
		...
		db_query("update users set firstname = :firstname,lastname = :lastname,PhoneNumber =:PhoneNumber,gender = :gender,date = :date,email = :email $image_query $password_query where id = :id limit 1",$arr);
